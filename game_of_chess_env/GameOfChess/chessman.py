...
class chessman:
    def __init__(self, name, color, x_pos, y_pos):
        self.name = name
        self.color = color
        self.pos = f"{x_pos}{y_pos}"
...
class rook(chessman):
    def validate_rook_move(self, next_pos, board):
        if(!self.validate_move(next_pos, board)):
            return False
        if(self.pos[0] == next_pos[0] || self.pos[1] == next_pos[1]):
            return True
        return False
...
class queen(chessman):
    def validate_queen_move(self, next_pos, board):
        if(!self.validate_move(next_pos, board)):
            return False
        if((self.pos[0] == next_pos[0] || self.pos[1] == next_pos[1]) and self.pos != next_pos):
            return True
        if(abs(ord(next_pos[0]) - ord(self.pos[0])) == abs(int(next_pos[1]) - int(self.pos[0]))):
            return True
        return False
